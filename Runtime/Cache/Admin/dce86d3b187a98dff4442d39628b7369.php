...
<div id="content-header">
	<h1>用户列表</h1>
	<div class="btn-group">
		<a href="<?php echo U('/Admin/User/userAdd');?>" class="btn btn-large" title="新增用户"><i class="fa fa-file"></i></a>
	</div>
</div>
<div id="breadcrumb">
					...
					<table class="table table-bordered table-striped table-hover">
						...
						<tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($vo["u_id"]); ?></td>
								<td><?php echo ($vo["u_name"]); ?></td>
								<td>
								<?php switch($vo['u_class']): case "1": ?>最高管理员<?php break;?>
								    <?php case "2": ?>普通管理员<?php break;?>
								    <?php default: ?>普通用户<?php endswitch;?>
								</td>
								<td><?php echo ($vo["u_email"]); ?></td>
								<td><?php echo (getTime($vo["u_time"])); ?></td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('/Admin/User/userEdit',array('id'=>$vo['u_id']));?>">编辑</a>&nbsp;&nbsp;
  	...
  	$(".deluser").click(function(){
  		...
  		$.ajax({
  			...
  			success: function(data){
  				if(data.error == 0){
  					succ("删除成功",data.msg,"/index.php/Admin/User/userList.html");
  				}else{
  					err(data.msg);
