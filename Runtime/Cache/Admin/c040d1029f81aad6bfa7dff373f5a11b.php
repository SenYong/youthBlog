					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
						  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<?php switch($vo['a_show']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
                                       <?php case "1": ?><span class=" btn-dark-red btn-xs">显示</span><?php break;?>
                                       <?php case "2": ?><span class=" btn-dark-red btn-xs">显示并推荐</span><?php break; endswitch;?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('/Admin/Article/ArtEdit',array('id'=>$vo['a_id']));?>">编辑</a>&nbsp;&nbsp;
  	...
  	$(".delsay").click(function(){
  		...
  		$.ajax({
  			...
  			success: function(data){
  				if(data.error == 0){
  					succ("删除成功",data.msg,"/index.php/Admin/Article/ArtList.html");
  				}else{
                    err(data.msg);
