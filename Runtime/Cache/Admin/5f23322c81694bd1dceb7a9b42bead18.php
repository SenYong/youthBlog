					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><?php echo (msubstr(strip_tags($vo["s_content"]),0,120,'utf-8',true)); ?></td>
									<td><?php echo (getTime($vo["s_time"])); ?></td>
								
									<td>
										<?php switch($vo['s_view']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
											<?php case "1": ?><span class=" btn-dark-green btn-xs">显示</span><?php break;?>
											<?php default: ?>暂无数据<?php endswitch;?>
									</td>
									<td>
										<a class="btn btn-success btn-sm" href="<?php echo U('Admin/Say/sayEdit',array('id'=>$vo['s_id']));?>">编辑</a>&nbsp;&nbsp;
	...
	$(".delsay",this).click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/index.php/Admin/Say/sayList.html");
				}else{
					err("错误!",data.msg);
