					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
							<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($vo["g_id"]); ?></td>
								<td><?php echo ($vo["g_name"]); ?></td>
								<td><?php echo (reFace($vo["g_content"])); ?></td>
								<td><?php echo (getTime($vo["g_time"])); ?></td>
								<td>
									<?php if(empty($vo['g_rtime'])): ?><span class=" btn-dark-red btn-xs">未处理</span>
									<?php else: ?>
										<span class=" btn-dark-green btn-xs">已处理</span><?php endif; ?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U('Admin/Gust/gustEdit',array('id'=>$vo['g_id']));?>">编辑</a>&nbsp;&nbsp;
	...
	$(".delgust",this).click(function(){
		...
		$.ajax({
			...
			success:function(data){
				if(data.error==0){
					succ("完成!",data.msg,"/index.php/Admin/Gust/gustList.html");
				}else{
					err("错误!",data.msg);
