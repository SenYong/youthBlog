...
<div id="content">
	<div id="content-header">
		<h1>链接列表</h1>
		<div class="btn-group">
			<a href="<?php echo U('/Admin/Link/index');?>" class="btn btn-large" title="新增链接"><i class="fa fa-file"></i></a>
		</div>
	</div>
	<div id="breadcrumb">
					...
					<table class="table table-bordered table-striped table-hover with-check">
						...
						<tbody>
						  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($vo["l_id"]); ?></td>
								<td><?php echo ($vo["l_name"]); ?></td>
								<td><?php echo (msubstr($vo["l_content"],0,80,'utf-8',true)); ?></td>
								<td>
									<?php switch($vo['l_show']): case "0": ?><span class=" btn-dark-red btn-xs">不显示</span><?php break;?>
									    <?php case "1": ?><span class=" btn-dark-green btn-xs">首页显示</span><?php break;?>
									    <?php case "2": ?><span class=" btn-dark-green btn-xs">内页显示</span><?php break; endswitch;?>
								</td>
								<td><a class="btn btn-success btn-sm" href="<?php echo U('/Admin/Link/linkEdit',array('id'=>$vo['l_id']));?>">编辑</a>&nbsp;&nbsp;
  	...
  	$('.delL').click(function(){
  		...
  		$.ajax({
  			...
  			success: function(data){
  				if(data.error == 0){
  					succ("成功",data.msg,"/index.php/Admin/Link/linkList.html");
  				}else{
  					err(data.msg);
