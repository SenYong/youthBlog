					...
					<table class="table table-bordered table-striped table-hover ">
						...
						<tbody>
						  <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td class="id"><?php echo ($vo["t_id"]); ?></td>
								<td><?php echo ($vo["t_name"]); ?></td>
								<td><?php echo ($vo["t_remark"]); ?></td>
								<td><?php echo (getTime($vo["t_time"])); ?></td>
								<td>
									<?php switch($vo['t_view']): case "0": ?><span class=" btn-dark-red btn-xs"></eq> >不显示</span><?php break;?>
										<?php case "1": ?><span class=" btn-dark-green btn-xs">显示</span><?php break;?>
										<?php default: ?>暂无数据<?php endswitch;?>
								</td>
								<td>
									<a class="btn btn-success btn-sm" href="<?php echo U( '/Admin/Tag/tagEdit', array('id'=>$vo['t_id']) );?>">编辑</a>&nbsp;&nbsp;
   ...
   $(document).ready(function(){
   	 $('.deltag').click(function(){
            ...
            success: function(data){
            	if(data.error == 0){
            		succ("删除成功",data.msg, "/index.php/Admin/Tag/tagList.html");
            	}else{
            		err(data.msg);
